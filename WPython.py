...
# === Read data ===
df = pd.read_excel('WPython.xlsx')  # Replace with your file path
angle_step = 15
radius_step = 0.05  # Adjust according to data precision
...
# Create interpolation grid (radius from 0 to 2.5)
theta_grid = np.linspace(0, 2 * np.pi, 360)
radius_grid = np.linspace(0, 2.5, 300)
T, R = np.meshgrid(theta_grid, radius_grid)

# Interpolation (use nearest to avoid blank areas)
...
# Color map settings (gradient + multiple bands)
...
# Colorbar (clean ticks)
cb = fig.colorbar(c, ax=ax, pad=0.1, ticks=[-50, 0, 100, 150, 200, 300])
...
# Radius settings (0–2.5), hide tick labels
...
# Save figure
plt.savefig('DBTT_Polar_Radius0to2_5_HideYLabel.png', dpi=600, bbox_inches='tight')
plt.show()
